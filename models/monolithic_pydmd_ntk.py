...
class NTK():
    ...
    def fit(self,x,y):
        x_tmp=jnp.real(jnp.array(x))
        y_tmp=jnp.real(jnp.array(y))
...
class Model():
    def __init__(self):
        self.model=ParametricDMD(DMD(svd_rank=-1),POD('svd',rank=400),NTK())
...
max_params=np.max(params_train)
min_params=np.min(params_train)
params_train=(params_train-min_params)/(max_params-min_params)
params_test=(params_test-min_params)/(max_params-min_params)
