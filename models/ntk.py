...
class Model():
    def __init__(self,min_param,max_param):
        ...
        self.apply_fn = jit(apply_fn)
        self.kernel_fn = jit(kernel_fn, static_argnames='get')
        self.pod=POD('randomized_svd',rank=500)
    def fit(self,x,y):
        self.pod.fit(y)
        self.basis=self.pod.transform(y)
        y_tmp=self.pod.modes
        x_tmp=(jnp.array(x)-self.min_param)/(self.max_param-self.min_param)
        y_tmp=jnp.array(y_tmp)
        self.predict_fn=nt.predict.gradient_descent_mse_ensemble(self.kernel_fn, x_tmp,
                                                      y_tmp, diag_reg=1e-4)
    
    def predict(self,x):
        x_tmp=(jnp.array(x)-self.min_param)/(self.max_param-self.min_param)
        y_tmp=self.predict_fn(x_test=x_tmp, get='nngp',compute_cov=False) 
        return y_tmp@self.basis
...
max_param=np.max(params_train)
min_param=np.min(params_train)


num_times=data.V_train.shape[1]
num_spaces=data.V_train.shape[2]



u_train=data.V_train[:,:,:,0].numpy().reshape(data.V_train.shape[0],-1)
u_test=data.V_test[:,:,:,0].numpy().reshape(data.V_test.shape[0],-1)

u_model=Model(min_param,max_param)
u_model.fit(params_train,u_train)
rec_u_train=u_model.predict(params_train)
u_train=u_train.reshape(-1,num_times,num_spaces)
rec_u_train=rec_u_train.reshape(-1,num_times,num_spaces)
rel_u_train_error=loss_function(rec_u_train,u_train)
rec_u_test=u_model.predict(params_test)
u_test=u_test.reshape(-1,num_times,num_spaces)
rec_u_test=rec_u_test.reshape(-1,num_times,num_spaces)
rel_u_test_error=loss_function(rec_u_test,u_test)


v_train=data.V_train[:,:,:,1].numpy().reshape(data.V_train.shape[0],-1)
v_test=data.V_test[:,:,:,1].numpy().reshape(data.V_train.shape[0],-1)
v_model=Model(min_param,max_param)
v_model.fit(params_train,v_train)
rec_v_train=v_model.predict(params_train)
v_train=v_train.reshape(-1,num_times,num_spaces)
rec_v_train=rec_v_train.reshape(-1,num_times,num_spaces)
rel_v_train_error=loss_function(rec_v_train,v_train)
rec_v_test=v_model.predict(params_test)
v_test=v_test.reshape(-1,num_times,num_spaces)
rec_v_test=rec_v_test.reshape(-1,num_times,num_spaces)
rel_v_test_error=loss_function(rec_v_test,v_test)

p_train=data.V_train[:,:,:,2].numpy().reshape(data.V_train.shape[0],-1)
p_test=data.V_test[:,:,:,2].numpy().reshape(data.V_train.shape[0],-1)
p_model=Model(min_param,max_param)
p_model.fit(params_train,p_train)
rec_p_train=p_model.predict(params_train)
p_train=p_train.reshape(-1,num_times,num_spaces)
rec_p_train=rec_p_train.reshape(-1,num_times,num_spaces)
rel_p_train_error=loss_function(rec_p_train,p_train)
rec_p_test=p_model.predict(params_test)
p_test=p_test.reshape(-1,num_times,num_spaces)
rec_p_test=rec_p_test.reshape(-1,num_times,num_spaces)
